...
class MemoryBankModule(torch.nn.Module):
    ...
    def _dequeue_and_enqueue(self, batch: torch.Tensor, labels: torch.Tensor):
        ...
        Args:
            ...
            labels:
                The latest batch of keys' labels to add to the memory bank.
    ...
    def forward(
        ...
        Args:
            ...
            labels:
                The label corresponding to the output, intended for use in nearest neighbor purity calculations
            update:
                If update is true, the output and labels are queued after the memory bank is returned.
                Otherwise they are not queued.

        Returns:
            The output if the memory bank is of size 0,
            otherwise the output and the entries from the memory bank.
